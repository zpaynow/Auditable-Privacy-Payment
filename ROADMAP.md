...
### 追加：多链服务（2026-09-12 完成，本地链验证）

| 步骤 | 内容 | 验收 |
| --- | --- | --- |
| M.1 ✅ | 两个服务以 `solidity/deployments/*.json` 为链配置来源，`RPC_URL_<id>`、`OPERATOR_KEY_<id>`、`AUDITOR_SECRET_<id>`、费用参数按链后缀配置并回退到无后缀值；`CHAINS` 限定服务的链 | 启动时校验 RPC 链 ID、operator 白名单、审计密钥与部署一致 |
| M.2 ✅ | 数据库所有表加 `chain_id`；每条链独立的打包任务 / 索引任务与 Merkle 树 | |
| M.3 ✅ | 全部接口改为 `/chains/{chainId}/…`，新增 `GET /chains`；鉴权字符串本来就含链 ID 与合约地址，签名只对一条链有效 | 两个端到端脚本在新路径下通过 |
| M.4 ✅ | web 按当前钱包链调用服务，服务未覆盖该链时聚合器开关不可用、票据源回退扫链 | |
